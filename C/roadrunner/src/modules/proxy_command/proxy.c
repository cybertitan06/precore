...
/**
 * @brief Open the local port that will serve as the proxy for the server
...
static pid_t proxy(char *target_host, char *target_port, uint16_t local_port)
{
    ...
    // fork into a new child process with fork()
	...
	// setup epoll so we can pass traffic effectively
	// setup_epoll()
	
	//start passing traffic for the proxy
	//perform_proxy()
...
Response *proxy_command(Command *cmd)
{
	...
	//target_host_len = deserialize_target_host()
